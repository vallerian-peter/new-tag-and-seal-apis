...
class SaveFarmerRegistrationFormAction
{
    public function handle(Request $request)
    {
        try {
            ...
            //check farmer exists
            $exists = Farmer::where(['farmer_organization_membership' => $request->farmer_organization_membership])->orWhere(['phone_1' => $request->phone_1])->orWhere('email', $request->email ?? '')->get();

            if (count($exists) > 0) {
                DB::rollBack();
                return ['status' => false, 'code' => 100, 'message' => 'Organization number membiship, phone number or email already taken', 'data' => []];
            }
